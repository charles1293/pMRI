...
fig, axes = plt.subplots(1, len(noise_levels), figsize=(18, 5))
...
for idx, sigma in enumerate(noise_levels):
    ...
    axes[idx].set_title(f'σ = {sigma}', fontsize=20, pad=10)
    axes[idx].axis('off')

plt.suptitle('Différents niveaux de bruit (R=2)', fontsize=20)
plt.tight_layout()
...
fig, axes = plt.subplots(2, len(noise_levels), figsize=(20, 9.5))
...
for idx, sigma in enumerate(noise_levels):
    ...
    axes[0, idx].set_title(f'R=2, σ={sigma}', fontsize=20, pad=10)
    axes[0, idx].axis('off')

    # Bottom row: R=4
    im_r4 = axes[1, idx].imshow(np.abs(reduced_FoV_r4[:, :, 0]), cmap='gray')
    axes[1, idx].set_title(f'R=4, σ={sigma}', fontsize=20, pad=10)
    axes[1, idx].axis('off')

plt.suptitle('Comparaison R=2 vs R=4', fontsize=20)
plt.tight_layout()
...
for idx, sigma in enumerate(noise_levels):
    ...
    axes[0, idx].set_title(f'R=2, σ={sigma}\nSNR={snr2:.2f} dB', fontsize=22, pad=12)
    axes[0, idx].axis('off')

    axes[1, idx].imshow(np.abs(reconstructed_r4), cmap='gray')
    axes[1, idx].set_title(f'R=4, σ={sigma}\nSNR={snr4:.2f} dB', fontsize=22, pad=12)
    axes[1, idx].axis('off')

plt.suptitle("", fontsize=14, y=0.97)
fig.subplots_adjust(top=0.90, hspace=0.28, wspace=0.08)
...
fig, axes = plt.subplots(2, cols, figsize=(4.0 * cols, 9.8))

axes[0, 0].imshow(np.abs(ref), cmap='gray')
axes[0, 0].set_title('Reference', fontsize=20, pad=10)
axes[0, 0].axis('off')

axes[1, 0].imshow(np.abs(ref), cmap='gray')
axes[1, 0].set_title('Reference', fontsize=20, pad=10)
axes[1, 0].axis('off')

axes[0, 1].imshow(np.abs(baseline_r2), cmap='gray')
axes[0, 1].set_title(f'R=2, sans reg\nSNR={snr_baseline_r2:.2f} dB', fontsize=20, pad=10)
axes[0, 1].axis('off')

axes[1, 1].imshow(np.abs(baseline_r4), cmap='gray')
axes[1, 1].set_title(f'R=4, sans reg\nSNR={snr_baseline_r4:.2f} dB', fontsize=20, pad=10)
axes[1, 1].axis('off')

for i, lam in enumerate(lam_values):
    axes[0, i + 2].imshow(np.abs(recon_lam_r2[i]), cmap='gray')
    axes[0, i + 2].set_title(f'R=2, λ={lam}\nSNR={snr_lam_r2[i]:.2f} dB', fontsize=20, pad=10)
    axes[0, i + 2].axis('off')

    axes[1, i + 2].imshow(np.abs(recon_lam_r4[i]), cmap='gray')
    axes[1, i + 2].set_title(f'R=4, λ={lam}\nSNR={snr_lam_r4[i]:.2f} dB', fontsize=20, pad=10)
    axes[1, i + 2].axis('off')

fig.suptitle('Tikhonov, σ=10: comparaison sans/avec regularisation', fontsize=20, y=0.98)
fig.subplots_adjust(top=0.90, hspace=0.34, wspace=0.16)
